...
class ortho_to_ipa:
    def __init__(self,load = True, datapath = 'data', fname_ortho='model/ortho.pt',fname_ipa='model/ipa.pt',fname_model='model/ortho_to_ipa.pt'):

        self.path = os.path.dirname(__file__)
        
        if load: 
            self.load_vocabs(os.path.join(self.path, fname_ortho),os.path.join(self.path, fname_ipa))
            self.load_model(fname_model)
        else:
            self.fpath = os.path.join(self.path, datapath)
